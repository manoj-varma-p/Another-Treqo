...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class Viscous extends ShaderPass {
      ...
      update(args = {}) {
        const { viscous: v, iterations, dt } = args;
        if (!this.uniforms) return;
    ...
    class WebGLManager {
      constructor(props) {
        this.props = props; this.lastUserInteraction = performance.now(); this.running = false;
  ...
  return (
    <div
      ref={mountRef}
      className={`liquid-ether-canvas ${className}`}
      style={{ width: '100%', height: '100%', position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', touchAction: 'none', ...style }}
    />
  );
}
